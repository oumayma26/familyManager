...
class LoginDialog(QDialog):
    ...
    def __init__(self, parent=None, is_first_setup: bool = False):
        ...
        self.setFixedSize(420, 480 if not is_first_setup else 520)
        self.setWindowFlags(
    ...
    def _setup_ui(self):
        ...
        # Quand il y a déjà des users, on ne montre PAS le lien "S'inscrire"
        if self.is_first_setup:
    ...
    def _do_login(self):
        ...
        if self.auth.login(username, password, self.remember.isChecked()):
    ...
    def _do_register(self):
        ...
        try:
            ...
            self.auth.login(username, password, remember=True)
            self.login_success.emit()
            self.accept()
