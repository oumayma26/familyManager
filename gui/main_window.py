#!/usr/bin/env python3
"""
main_window.py — Fenêtre principale Family Manager

Architecture :
- Sidebar gauche (navigation + liste des personnes)
- Zone centrale avec QStackedWidget (onglets : Détails, Arbre, Stats, Pensions)
- Style Linear-inspired (slate/indigo)
- Fermeture = logout automatique (suppression du token)
"""
...
from PySide6.QtGui import QFont, QIcon, QPixmap, QPainter, QBrush, QColor, QAction, QCloseEvent
...
from gui.pension_view import PensionView
...
from gui.changelog_dialog import get_latest_version
APP_VERSION = get_latest_version()


class MainWindow(QMainWindow):
    """Fenêtre principale de l'application Family Manager."""
    ...
    def __init__(self):
        ...
        self.setWindowTitle(f"Family Manager v{APP_VERSION}")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # FERMETURE → LOGOUT AUTOMATIQUE
    # ═══════════════════════════════════════════════════════════════════════════

    def closeEvent(self, event: QCloseEvent):
        """
        Appelé quand l'utilisateur clique sur le X de la fenêtre.
        Déconnecte la session et supprime le token persistant.
        """
        # Logout silencieux : supprime le token + nettoie la session
        from core.auth_manager import AuthManager
        auth = AuthManager()
        auth.logout()

        # Accepter la fermeture normalement
        event.accept()
    ...
    def _setup_menu_bar(self):
        """Configure la barre de menu (Aide → Nouveautés, À propos)."""
        menu_bar = self.menuBar()

        # Menu Aide
        help_menu = menu_bar.addMenu("Aide")

        changelog_action = QAction("✨ Nouveautés", self)
    ...
    def _build_sidebar(self) -> QFrame:
        """Construit la sidebar gauche (logo, recherche, liste, boutons)."""
        left_panel = QFrame()
        ...
        # --- Zone de recherche ---
        search_container = self._build_search_box()
        ...
        self.list_persons = self._build_persons_table()
        left_layout.addWidget(self.list_persons)

        # --- Boutons du bas ---
        self.btn_changelog = QPushButton("✨ Nouveautés")
        self.btn_changelog.setObjectName("ghost")
        self.btn_changelog.setMinimumHeight(36)
        self.btn_changelog.clicked.connect(self._show_changelog)
        left_layout.addWidget(self.btn_changelog)
    ...
    def _build_search_box(self) -> QFrame:
        """Construit la zone de recherche avec filtres."""
    ...
    def _build_persons_table(self) -> QTableWidget:
        """Construit le tableau de liste des personnes."""
        table = QTableWidget()
        table.setColumnCount(3)
        table.setHorizontalHeaderLabels(["", "Nom", ""])
        ...
        table.cellClicked.connect(self.on_person_selected_table)
        return table

    def _build_content_area(self) -> QFrame:
        """Construit la zone de contenu droite (onglets + stacked widget)."""
        right_panel = QFrame()
        ...
        # --- Header avec onglets ---
        header_container = self._build_tab_bar()
    ...
    def _build_tab_bar(self) -> QFrame:
        """Construit la barre d'onglets (Détails, Arbre, Stats, Pensions)."""
        ...
        self.btn_pensions = QPushButton("💰 Pensions")
    ...
    def load_persons_list(self):
        """Charge la liste des personnes (avec filtres actifs)."""
        self.apply_filters()

    def on_person_selected_table(self, row: int, column: int):
        """Gère le clic sur une ligne du tableau."""
        if column == 2:  # Colonne suppression
    ...
    def select_person(self, person_id: int):
        """Sélectionne une personne et charge ses données."""
    ...
    def show_add_form(self):
        """Affiche le formulaire vierge pour ajouter une personne."""
        self.current_person_id = None
        self.person_form.clear_form()
        self.switch_tab("details")
        self.list_persons.clearSelection()

    def on_person_saved(self, person_id: int):
        """Callback après sauvegarde d'une personne."""
        self.load_persons_list()
        self.current_person_id = person_id
    ...
    def on_family_member_added(self):
        """Callback après ajout d'un membre de famille."""
        if self.btn_tree.isChecked():
            self.tree_view.load_tree(self.current_person_id)
    ...
    def apply_filters(self):
        ...
        if search_text:
            persons = [
                p for p in persons
                if search_text in (p.get('first_name') or '').lower()
                or search_text in (p.get('last_name') or '').lower()
                or search_text in (p.get('cin') or '').lower()
            ]

        if gender_filter:
            persons = [p for p in persons if p.get('gender') == gender_filter]

        self._populate_persons_table(persons)

    def _populate_persons_table(self, persons: list):
        """Remplit le tableau avec les personnes filtrées."""
    ...
    def create_default_avatar(self, gender: str) -> QIcon:
        """Crée un avatar par défaut selon le genre."""
        size = 32
    ...
    def _show_changelog(self):
        ...
        # Affiche TOUTES les features de TOUTES les versions
        all_features = []
        for ver, feats in CHANGELOG.items():
            all_features.extend([f"[v{ver}] {f}" for f in feats])

        dialog = ChangelogDialog(get_latest_version(), all_features)
        dialog.exec()
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            self,
            "À propos de Family Manager",
            f"<h2>Family Manager</h2>"
            f"<p>Version {APP_VERSION}</p>"
            "<p>Gestion des familles de martyrs et pensions.</p>"
            "<p>© 2024 Family Manager Team</p>"
        )