...
# On utilise hashlib en natif (pas besoin de bcrypt externe)
# Si tu veux bcrypt plus robuste : pip install bcrypt
...
class AuthService:
    ...
    PEPPER = "FamilyManager2024_SecretPepper_v1"  # Change ça ! Ne jamais exposer.
    SESSION_DAYS = 30
    MIN_PASSWORD_LENGTH = 6
    
    def __init__(self, db_path: str = "database/family_manager.db"):
    ...
    def _hash_password(self, password: str, salt: Optional[bytes] = None) -> str:
        """
        Hash un mot de passe avec salt + pepper.
        Format stocké : salt_hex:hash_hex
        """
        if salt is None:
            salt = os.urandom(32)  # 256 bits de salt
        
        # Combinaison : pepper + salt + password
        key = hmac.new(
            self.PEPPER.encode(),
    ...
    def update_password(self, user_id: int, old_password: str, new_password: str) -> bool:
        ...
        if len(new_password) < self.MIN_PASSWORD_LENGTH:
            raise AuthError(f"Nouveau mot de passe trop court")
    ...
    def delete_user(self, user_id: int) -> bool:
        """Supprime un utilisateur (et ses sessions)"""
        with sqlite3.connect(self.db_path) as conn:
    ...
    def validate_session(self, token: str) -> Optional[User]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Rafraîchir la session (optionnel)
    ...
    def create_first_admin(self, username: str, password: str) -> User:
        """Crée le premier compte admin (setup initial)"""
        if self.has_users():
